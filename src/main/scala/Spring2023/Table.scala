...
case class Table[T](xs: List[T]) {
   ...
   * @return a new Table[T] with only the matching rows.
   */
  def filter(p: T => Boolean): Table[T] = lensFilter(p)(identity)
   ...
   */
  def sample(n: Int)(implicit r: Random): Table[T] = Table(r.shuffle(xs).take(xs.length / n))

    // Table(r.shuffle(xs).take(xs.length / n))
  /**
  ...
  def sum(implicit ev: Numeric[T]): Double = {
    xs.foldLeft(0.0){ case(a, b) => a +  ev.toDouble(b) }
  }
}
...
object Table {
   ...
   * @return a Table[T].
   */
  def apply[T](xs: LazyList[T]): Table[T] = Table(xs.take(100000).toList)
